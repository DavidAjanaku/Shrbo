...
const TestimonialInfo = ({ reviews }) => {
  ...
  const rating_category = ratinginfo.map((cat) => (
    <div
      key={cat.id}
      className=" relative px-2   md:mr-[8.33%]  w-full  md:px-2 "
    >
  ...
  const clients = [
    {
      ...
      date: "august 28 2023",
      min: adv,
      header: '"They’re amazing plumbers"',
      review:
  ...
  const slides = reviews.map((client) => (
  ...
  return (
    <div className="w-full h-full flex flex-col lg:flex-row ">
      <div className=" mx-auto md:w-1/2   box-border block text-base  ">
        ...
        <div className=" flex flex-row   items-center mb-8   w-full ">
          <div className=" rounded-md bg-slate-100 flex flex-col items-center p-4 md:p-6 gap-1 text-center  ">
            ...
            <div>
              <span className="text-orange-400 mr-1 text-lg md:text-2xl">
                &#9733; &#9733; &#9733; &#9733;
              </span>
            </div>
      ...
      <div className="  h-full md:overflow-y-scroll md:w-1/2 example">
        <div className=" w-full mb-8 uppercase ">
          <span>Comments From Verified Guests </span>
        </div>
        <div> {slides} </div>
