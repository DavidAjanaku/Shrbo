  ...
  const fetchData = async () => {
    ...
    await axios.get(`/hostCompletedPayoutsHistory?${qs.stringify(getRandomuserParams(tableParams))}`).then(response => {
      const results = response.data.data.map(item => ({
        ...
        amountPaid: formatAmountWithCommas(item.paymentAmount),
        paymentStatus: 'Completed',
        hostname: item.hostname,
        securityFee: formatAmountWithCommas(item.securityFee),
        duration_of_stay: `${item.duration_of_stay} Night(s)`,
        amountForOneNight: ` ${formatAmountWithCommas(item.amountForOneNight)}`,
        serviceFee: formatAmountWithCommas(item.serviceFee),
